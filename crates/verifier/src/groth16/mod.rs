...
impl Groth16Verifier {
    pub fn batch_verify(
        ...
        proofs.iter().try_for_each(|proof| {
            if proof.len() < VK_HASH_PREFIX_LENGTH {
                Err(Groth16Error::GeneralError(Error::InvalidData))
            } else if groth16_vk_hash != proof[..VK_HASH_PREFIX_LENGTH] {
        ...
        let public_inputs_vec: Vec<[[u8; 32]; 2]> = sp1_public_inputs
            .iter()
            .map(|input| {
                [
                    sp1_vkey_hash,             // [u8; 32]
                    hash_public_inputs(input), // [u8; 32]
                ]
            })
            .collect();
        let public_inputs: Vec<&[[u8; 32]]> = public_inputs_vec
            .iter()
            .map(|arr| arr.as_ref()) // Convert &[[u8; 32]; 2] -> &[[u8; 32]]
            .collect();

        Self::batch_verify_gnark_proof(proofs, &public_inputs, groth16_vk)
    }
    ...
    pub fn batch_verify_gnark_proof(
        proofs: &[&[u8]],
        public_inputs: &[&[[u8; 32]]],
        groth16_vk: &[u8],
    ) -> Result<(), Groth16Error> {
        let proofs: Vec<_> = proofs
            .iter()
            .map(|proof| load_groth16_proof_from_bytes(&proof[VK_HASH_PREFIX_LENGTH..]))
            .collect::<Result<_, _>>()?;

        let groth16_vk = load_groth16_verifying_key_from_bytes(groth16_vk)?;

        let public_inputs: Vec<Vec<Fr>> = public_inputs
            .iter()
            .map(|inputs| inputs.iter().map(|input| Fr::from_slice(input).unwrap()).collect())
            .collect();
        let public_inputs: Vec<&[Fr]> = public_inputs.iter().map(|v| v.as_slice()).collect();

        batch_verify_groth16_algebraic(&groth16_vk, &proofs, &public_inputs)
    }
}
