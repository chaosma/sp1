...
use sp1_sdk::{
    action::{compress_all_proofs, run_recursion_first_layer, run_recursion_two_to_one},
    SP1Proof, SP1ProofCommonData, SP1ProofWithPublicValues,
};
use std::fs::File;
use std::io::Read;
use std::path::Path;
...
fn main() {
    ...
    } else if let Some(index) = args.first_layer {
